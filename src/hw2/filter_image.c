...
// Answer: We should use preserve for the sharpen, emboss, and make_box filters because our
// convolution will return an image with the same number of channels as the input image when
// preserve is used. Using the dog image, if we use preserve with the sharpen filter, our
// convolution will return a sharpened dog image in RGB. If we use preserve with the emboss
// filter, our convolution will return a dog image in with the extra stylings. Using preserve
// in make_box_filter would blur the image, which should have color (not be in grayscale),
// hence we should use preserve.We would not use preserve for the highpass filter because the
// highpass filter finds edges. When preserve is not used, our convolution returns a 1
// channel image, which is in grayscale. When using a highpass filter to find edges, it will
// be easier to see the edges in a grayscale image where the edges are white and other parts
